...
import generator.atoms.TokensHolder;
...
public class LexGenerator {
    ...
    LexGenerator(Grammar grammar) throws IOException {
        ...
        String fileName = grammar.grammarName + "Lex.java";
    ...
    private StringBuilder fileText() {
        ...
        // class
        sb.append("\n");
        sb.append(printString("public class " + grammar.grammarName + "Lex {", 0));
        ...
        // constructor
        sb.append(printString("\tpublic " + grammar.grammarName + "Lex(String input) throws Exception {\n" +
                "\t\tinputSB = new StringBuilder(input);", 0));

        sb.append("\n");

        for (TokensHolder item : grammar.tokens) {
            sb.append(printString("tokenPatterns.add(new TokensHolder(" + grammar.grammarName + "Token." + item.name + "," + item.pattern + "));", 2));
        }

        sb.append("\n");

        for (TokensHolder item : grammar.skipTokens) {
            sb.append(printString("skipTokenPatterns.add(new TokensHolder(" + grammar.grammarName + "Token." + item.name + "," + item.pattern + "));", 2));
        }
